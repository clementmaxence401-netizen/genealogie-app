...
st.caption("Tous les sites essentiels pour rechercher vos ancêtres, organisés par région, pays et type")

st.divider()

# Configuration responsive de la barre de filtres
col_search, col_cat, col_pays, col_type, col_cost = st.columns([2.0, 1.5, 1.5, 1.5, 1.2])

with col_search:
    query = st.text_input("🔍 Rechercher", placeholder="Nom d'un site, pays, sujet…", label_visibility="collapsed")

with col_cat:
    categorie = st.selectbox("Région / Thème", CATEGORIES, label_visibility="collapsed")

# Extraction dynamique de la liste des pays disponibles selon la catégorie sélectionnée
if categorie == "Toutes":
    liste_pays = ["Tous"] + sorted(list(set(s["pays"] for s in SITES if s["pays"] != "Multi")))
else:
    liste_pays = ["Tous"] + sorted(list(set(s["pays"] for s in SITES if s["cat"] == categorie and s["pays"] != "Multi")))

with col_pays:
    pays_selectionne = st.selectbox("Pays spécifique", liste_pays, label_visibility="collapsed", disabled=(len(liste_pays) <= 1))
...
def filtrer(sites):
    ...
    for s in sites:
        if categorie != "Toutes" and s["cat"] != categorie:
            continue
        if pays_selectionne != "Tous" and s["pays"] != pays_selectionne:
            continue
        ...
        if query:
            q = query.lower()
            if q not in s["name"].lower() and q not in s["desc"].lower() and q not in s["pays"].lower():
                continue
        result.append(s)
    return result
...
if not sites_filtres:
    st.warning("Aucun résultat. Essayez d'autres critères de recherche ou élargissez vos filtres.")
else:
    cols = st.columns(2)
    for i, site in enumerate(sites_filtres):
        ...
        # Badge pays spécifique si disponible (sauf si global/multi)
        pays_tag = f" 📍 {site['pays']}" if site["pays"] != "Multi" else ""
        ...
        <div class="card">
            ...
            <div class="card-footer">
                <span class="badge badge-{badge_cls}">{badge_emoji} {site['cat']}{pays_tag}</span>
